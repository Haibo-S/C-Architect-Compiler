...
int main() {
  ...
  //cout << endl;
  //cout << dfa.getNextState("start", '\n') << endl;
  //cout << dfa.getNextState("ZERO", 'x') << endl;
  //cout << dfa.getNextState("NUM", 'c') << endl;
  //cout << simple_MM("\n", dfa) << endl;
...
string simple_MM(string s, DFA& dfa){
  ...
  if(nextState == "DECINT" && !isWithinRange(s)){
...
bool isWithinRange(const std::string& num){
  const string max_p = "4294967295";
  const string max_n = "-2147483648";
